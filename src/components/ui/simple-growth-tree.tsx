...
export function SimpleTree({
  ...
  const createTree = (width: number, height: number): Tree => {
    ...
    const tree: Tree = {
      ...
      teinte: random(treeHueMin, treeHueMax),
      index: 0,
    ...
    if (branch.age >= Math.floor(branch.maxlife / branch.gen) || random(1) < 0.025 * branch.gen) {
      ...
      if (branch.stw > 0.4 && branch.gen < 5) {
        ...
        if (random(1) < branch.proba1 / Math.pow(branch.gen, 0.9)) {
          brs.push(
            createBranch(pos, branch.stw * random(0.5, 0.75), branch.angle + random(0.6, 1.0) * branch.deviation, branch.gen + 0.2, branch.index, tree),
          )
        }
        ...
        if (branch.gen < 3 && random(1) < branch.proba3 / Math.pow(branch.gen, 1.1)) {
          brs.push(
            createBranch(pos, branch.stw * random(0.6, 0.8), branch.angle + random(0.3, 0.7) * branch.deviation, branch.gen + 0.15, branch.index, tree),
          )
        }
  ...
  const setup = useCallback(() => {
    ...
    if (bgColor) {
      ...
      ctx.fillStyle = hsbToRgb(42, 12, 248)
    }
